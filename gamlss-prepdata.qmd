...
Partitioning the data is essential for comparing different models and checking for overfitting. Overfitting occurs when a model fails to generalize well to new data, typically because it is too closely fitted to the data. In contrast, underfitting refers to a poor model that fails to capture the essential patterns in both fitted and the new data, resulting to a model that is far the population intended to represent. Note that the fitted data  It commonly to  refer to data used for fitting the model  as **training** data and the newdata as **test** data. 

@fig-data_split shows different ways the data can be partitioned. A dataset can be split once, in **holdout** samples, or split multiple times using cross validation, **CV**, or **bootstrap**. For large datasets, it is common to split the data into a _training_ set and two holdout samples, the _test_ set, and, if necessary, the _validation_ set. The training set is used for fitting the model, the test set is used to evaluate the prediction power and therefore to compare models, and the validation set is used for tuning the model’s `hyperparameters`. [Note that for additive smoothing regression models,  the hyper parameters are the `smoothing` parameters.] Observations in the training set are often referred to as the `in-bag`  data, while observations in the test or validation set are termed the `out-of-bag` data. 


Partitioning the data into training, test, and validation sets should be done randomly. That bring us to the two possible problems which could occurred during splitting the data; The **zero variance variable** problem and the **extreme values problem**. The zero variance variable (or nearly zero variance problem) occurs when a variable in the data (especially a factor) has only few distinct values and when the frequency of say one of the values is very low.  For example, let us consider this rather extreme case in which  we have the factor `gender`  with mostly `female` values and only few `males`. In any random portioning, all males could end up in only one of the partitions make the model fitting and interpretation difficult. One should have at least a warning if this this the case.  The extreme values problem is similar in nature but could effect the judgement in  overfitting. Note that  extreme values could occurred in both the explanatory and the response variables.  Since we have deal with `outliers` in the x's in  @sec-Outliers let us concentrate  to extreme values in the response. Extreme values in the response  (if they  genuine) are of  great importance in the analysis for the determination of the risk. Potentially such evens when splitting the data can go to one partition and not to the other. Any evaluation of risks in this case could be potentially wrong. A possible solution to this problem is the **repeated**  partition of the data.    

Bootstrap , in the middle of @fig-data_split, is a repeated  partition method. In the classical non-parametric bootstrap the index of the data i.e. $1,2,\ldots,n$ is sampled with replacement $B$ times and the model is refitted the same number of times. The Bayesian bootstrap re-weights the data as samples from a multinomial distribution and fits the re-weighted models $B$ times. So in both cases the repetition is $B$.
While both bootstraps are computationally expensive, provides additional insights into the model and in particular regarding the variability of all of its parameters. This because a sample of length $B$ is obtained for all the parameters of interest. 
...
::: callout-note
Apart from bootstrapping repeated partition can be performed in both holdout samples and k-fold cross validation mitigating some of the problems created from single partitions.
:::
...
Data partitioning help in model performance evaluation, in model assumption checks, and for  detection of overfitting.  Here are some rules of thump applied to  distributional regression models;

- if the residuals from the training data and the test data sets behave well, one is confidence, that the distributional assumption is adequate. Also if the  and that there is no important variable is left from the data.
...
All methods in @fig-data_split, need  performance metrics which would answer problem of interest. Those metrics can help can: i) facilitate model comparison , ii) detect overfitting, and iii) select  hyper-parameters.
...
::: callout-note
Note that the common measure of goodness-of-fit used in machine learning models  used with data partition are **not** appropriate for distributional regression model like  GAMLSS. This  because those measures concentrate in the behaviour at the centre of the distribution of the response which may or not be the focus of the study.
:::

The measures of goodness-of-fit used machine learing models with partitioning the data are;
...
All the measures of goodness-of-fit, above,  are NOT generalize well when the distribution of the response is not symmetric (like the Normal distribution), and  also when the focus of the study is not at the location parameter (mean or median) of the distribution of the response. If, for example, the focus of the study is on the tail of the distribution (i.e how often a threshold is bridged) those measure are not appropriate. For  distributional regression model the following appropriatre measures of goodness-of-fitand can be use in both the  in bag and out of bag data sets;
...
The function `data_boot_index()` and `data_Kfold_index()`  creates  appropriate vector for indexing, while `data_boot_weights()` and  `data_Kfold_weights()` generates the appropriate vectors for prior weights.  
