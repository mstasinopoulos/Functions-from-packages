...
-  Transform the variables into suitable **R** classes 
...
| [`data_na_obs()`](#sec-data-na-obs) | Displays the count of NA (missing) values for each variable in the dataset |
...
| [`y_outliers_both()`](#sec-y-both-outliers) | identify possible outliers using both z-scores and quantile rule methodologies|
...
Outliers in the continuous explanatory variables can potentially affect curve fitting, whether using linear or smooth non-parametric terms. In such cases, removing outliers from the x-variables can make the model more robust. Outliers are observations that deviate significantly from the rest of the data, but note that the concept also depends on the dimension being considered. An single observation value might be an outlier within an explanatory variable. A pair observation might be an outlier examining within a pair-wise relationships. We identify extreme observations on  individual continuous variables, based on how far they lie from the majority of the data.  Pairwise plots of continuous variables may identify  outliers in two-dimensions. Leverage points are useful to identify outliers in the $r$ dimensions where $r$ is the length of the x-variables. 

::: callout-note
At this preliminary stage of the analysis (no model has been fitted yet), it is difficult to identify outliers among `factors`. This is because outliers in factors typically do not appear unusual when examining individual variables but only in combination of other factors or variables. Identifying `factor` outliers is a task better suited for the modelling stage of the analysis.
:::

The function `y_outliers()` in @sec-y-outliers, applied  only to continuous variables, uses two methodologies (option `type`) to identify outliers; 
...
Both methodologies are trying to remove extreme skewness in the variables before looking for outliers by using the (internal) function `y_Ptrans()`.  The function is applied to any variable with  positive values. The assumption,  is that positive variables are more likely to be  highly skewed.  The function `y_Ptrans()`  attempts to find the power $\lambda$, of a power transformation $T = X^\lambda$, that minimizes the Jarque-Bera test statistic—a measure of deviation from normality. [The Jarque-Bera test statistic is always non-negative and a value further than zero indicates stronger evidence against normality.] The goal of this methodology is to reduce extreme skewness in the data before identify the outliers. If the data are not extremely 
skewed the power transformation will do no harm.  


Now given that the positive variables are transformed (using the power transformation) and that all the rest of variables remain untransformed, the z-scores methodology fits a four parameter distribution to the variable (with default the `SHASHo` distribution) and computes the residuals of the fitted model (z-scores).  The z-scores methodology identify outliers if a z-score  have an absolute value bigger that a predetermining  value say $K$. The  quantile rule methodology identify outliers by doing something similar. It identify an  observation if it  is further than $K \times MAD$ way from the median.  The value of $K$ is set by the option `value` of the function `y_outliers()`. But if the `value` is missing it is calculated using the formulas $\Phi^{-1}(1/(10 \times N))$, where N is the number of observations. The methodology is appropriate for identifying outliers in one dimension. To identify outliers in $p$ dimensions the function `data_leverage()`, see @sec-data-leverage-1, can be used instead.

::: callout-note
Several of the graphical functions described in @sec-Graphical-functions are good for identify  outliers visually. In fact we recommend that the function `data_outliers()` and its related function  `data_outliers_both()` and `data_outliers_by()` should be used in combination with functions like `data_plot()` or `data_zscores()` or `y_dots()` of the package `gamlss.gplots`.
:::
...
-  `zscores`: A  distribution is fitted to the data (default `SHASHo`) then the residuals (z-scores) are taken and observations with large residuals are identified.

- `quantile`: This is the classical methodology and it is based on sample quantiles.  An observation is identified if it far from the median. How far usually is taken  as $K$ time the MAD.  
...
Note that the function `y_outliers()` is used by `data_outliers()` to identify outliers in all continuous variables in the data.
...
The function `y_outliers_both()` uses both `zscores` and `quantile` methodology and reports the observations appearing either in the `method=intersection` (default) or in the  `method=union`. 
...
#### `y_outliers_by()` {#sec-y-both-by}

The function `y_outliers_by()` splits the data, before it performes the detection of outliers, **by** a given factor. It is useful if the given factor split the data in a meaningful ways. Continuous variable cab be split in advange using the function `cut()`. 
