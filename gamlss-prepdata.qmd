...
| [`y_both_outliers()`](#sec-y-both-outliers) | identify possible outliers using both z-scores and quantile rule methodologies|
...
Both methodologies are trying to remove extreme skewness in the variables before looking for outliers by using the (internal) function `x_Ptrans()`.  The function is applied to any variable with  positive values. The assumption,  is that positive variables are more likely to be  highly skewed.  The function `x_Ptrans()`  attempts to find the power $\lambda$, of a power transformation $T = X^\lambda$, that minimizes the Jarque-Bera test statistic—a measure of deviation from normality. [The Jarque-Bera test statistic is always non-negative and a value further than zero indicates stronger evidence against normality.] The goal of this methodology is to reduce extreme skewness in the data before identify the outliers. If the data are not extremely 
skewed the power transformation will do no big harm.  


Now given that the positive variables are transformed (using the power transformation) and that all the rest pf variables remain untransformed, the z-scores methodology fits a four parameter distribution to the variable (the default is the `SHASHo` distribution) and computes the residuals (z-scores).  The z-scores methodology identify outliers if a  z-scores have have absolute value bigger that a predetermining  point  while the quantile rule methodology identify outliers if there are further than $K \times SIR$ way from the median.  The predetermining value and the $K$ are set by the option `value` of the function `y_outliers()`. Note that if `value` is missing it is calculated using the formulas $\Phi^{-1}(1/(10 \times N))$, where N is the number of observations. The methodology is appropriate for identifying outliers in one dimension. To identify outliers in $p$ dimensions the function `data_leverage()`, see @sec-data-leverage-1, can be used.
