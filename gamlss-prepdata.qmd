...
| [`data_Ptrans_plot()`](#data-Ptrans-plot) | plotting all x's agains the response using identity, square root and log transformations |
...
The function `y_outliers_z()` tries to put the  fdunctions `y_outliers_loop()` and `y_outliers_by()`. It works only for the z-scores methodology but al the four combination of 
`by` and `loop` can be used. m      
