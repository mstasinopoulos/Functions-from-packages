...
Partitioning the data into training, test, and validation sets should be done randomly. This bring us to the two possible problems could occurred when splitting the data; The **zero variance variable** problem and the **extreme values problem**. The zero variance variable (or nearly zero variance problem) occurs when a variable in the data (especially a factor) has only few distinct values and the frequency  one of the values is very low.  For example, if  we have the factor `gender` with mostly `female` values and only few values `males` and random portioning the data,  may all males could end up in only one of the partitions. That is, one of the partition could  have gender properly with two levels while the other partition with only one level.   This would make the fitting and interpretation of the model difficult. One should check if this is the case.  The extreme values problem is similar in nature but could effect our overfitting  judgement. [xtreme values could occurred in both the explanatory and the response variables.  We have deal with `outliers` in the x's in  @sec-Outliers let us concentrate now on extreme values in the response. Extreme values in the response  (if they  genuine) are of great importance in the risk analysis. Potentially such evens when splitting the data can go to one partition and not to the other. Any evaluation of risks in this case could be potentially wrong. With **repeated**  partition of the data the problem couldd be eliminated,    
...
::: callout-note
Repeated partitions performed in both bootstrapping and k-fold cross validation could mitigating some of the problems assocated from single partitions.
:::
